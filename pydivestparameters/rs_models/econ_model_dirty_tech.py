...
class econ_model():
    ...
    R, G, Cc, Cd = sp.symbols('R, G, C_c C_d', positive=True, real=True)
    eta, s, delta, chi, alpha, betac, betad, gammac, gammad = \
        sp.symbols('eta s delta chi alpha beta_c beta_d gamma_c gamma_d',
    ...
    s2 = {
        Xc: (bc * Kc**betac * Cc**gammac)**(1. / (1 - alpha)),
        Xd: (bd * Kd**betad * Cd**gammad)**(1. / (1 - alpha)),
    ...
    c_outputs = {
        Yc: bc * Lc**alpha * Kc**betac * Cc**gammac,
        Yd: bd * Ld**alpha * Kd**betad * Cd**gammad,
        rc: rc,
        rd: rd,
        Cc: Cc,
        G: G
    ...
    c_kdots = {
        ...
        dG: -R,
        dCc: bc * Lc**alpha * Kc**betac * Cc**gammac - chi * Cc
        dCd: bd * Ld**alpha * Kd**betad * Cd**gammad - chi * Cd
    }
    ...
    def initial(self, Kc0, Kd0, Cc0, Cd0, G0=None):
        ...
        Cd0: float
            initial knowledge stock in the dirty sector
        G0: float
    ...
    def _eval(self, L, Kc, Kd, Cc, Cd, R, G, e):
        """evaluates model equations for given variable values
        """

        variables = (L, Kc, Kd, Cc, Cd, R, G, e)
    ...
    def _step(self):
        ...
        cc_t = self.tCc[self.t]
        cd_t = self.tCd[self.t]
        r_t = self.tR[self.t]
        g_t = self.tG[self.t]
        ...
        cc_tp1 = cc_t + dots[self.dCc]
        cd_tp1 = cd_t + dots[self.dCc]
        g_tp1 = g_t - r_t
